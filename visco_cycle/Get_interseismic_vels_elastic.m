function [Gss, Gds] = Get_interseismic_vels_elastic(Segs,dips,Ld,Cd,xystats,H1,get_SS,get_DS)
...
for k=1:size(centers,1)  %loop over number of segments
        ...
        m_ss=[SegLength(k) W Ld(k) dips(k) strike(k) bottcenters_locked(k,1) bottcenters_locked(k,2) 1 0 0];
        %dip-slip contribution 
        m_ds=[SegLength(k) W Ld(k) dips(k) strike(k) bottcenters_locked(k,1) bottcenters_locked(k,2) 0 1 0];
