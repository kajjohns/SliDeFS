function [Gss, Gds] = Get_ss_vels_cycle(Segs,dips,Ld,Cd,xystats,...
                      H1,get_SS,get_DS)
...
for k=1:size(centers,1)  %loop over number of segments
        ...
        m_ds=[SegLength(k) W H1 dips(k) strike(k) bottcenters(k,1) bottcenters(k,2) 0 1 0];
            ...
            if get_SS(k)  %only compute if requested -- it is expensive to compute and not use 
                ...
                [Ue,D,S,flag]=disloc3d(m_ss',xloc,1,.25); %elastic part
                Uss = Ue + LayeredBasis_viscous(m_ss,xloc,H1,1,1,10^6,1);  
            ...
            if get_DS(k)  %only compute if requested -- it is expensive to compute and not use 
                [Ue,D,S,flag]=disloc3d(m_ds',xloc,1,.25); %elastic part
                Uds = Ue + LayeredBasis_viscous(m_ds,xloc,H1,1,1,10^6,1);
            else
    ...
    Gds(:,k)=[Uds(1,:)';Uds(2,:)';Uds(3,:)'];
     ...
     disp(['completed ' num2str(k/size(centers,1)*100) '% of steady contribution.'])
